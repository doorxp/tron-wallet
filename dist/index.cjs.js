...
var sha3 = _interopDefault(require('js-sha3'));
...
var transactionBuilder = require('@tronscan/client/src/utils/transactionBuilder');
var crypto = require('@tronscan/client/src/utils/crypto');
...
class TronWallet {
  ...
  static fromTronPrivateKey (pk, isTestNet = false) {
    return new this({ privateKey: safeBuffer.Buffer(pk, 'hex'), isTestNet })
  }

  constructor ({ seed, extendedKey, privateKey, isTestNet }) {
    if (seed) {
      this._seed = seed;
      this._node = hdkey.fromMasterSeed(safeBuffer.Buffer(seed, 'hex'));
    } else if (extendedKey) {
      this._seed = null;
  ...
  updateTransaction (tx, latestBlock) {
    ...
    const signed = crypto.signTransaction(this.getTronPrivateKey(), transactionWithRefs);
  ...
  generateTransaction (to, amount, token = 'TRX', latestBlock) {
    const transaction = transactionBuilder.buildTransferTransaction(token, this.getAddress(), to, amount);
    return this.updateTransaction(transaction, latestBlock)
  }

  updateAccount (name, latestBlock) {
    const transaction = transactionBuilder.buildAccountUpdate(this.getAddress(), name);
    return this.updateTransaction(transaction, latestBlock)
  }

  freeze (amount, duration = 3, latestBlock) {
    const transaction = transactionBuilder.buildFreezeBalance(this.getAddress(), amount, duration);
    return this.updateTransaction(transaction, latestBlock)
  }

  unfreeze (latestBlock) {
    const transaction = transactionBuilder.buildUnfreezeBalance(this.getAddress());
    return this.updateTransaction(transaction, latestBlock)
  }

  vote (votes, latestBlock) {
    const transaction = transactionBuilder.buildVote(this.getAddress(), votes);
    return this.updateTransaction(transaction, latestBlock)
  }

  issueAssets (options, latestBlock) {
    const transaction = transactionBuilder.buildAssetIssue(options, latestBlock);
    return this.updateTransaction(transaction, latestBlock)
  }

  buyAssets (issuer, token, amount, latestBlock) {
    const transaction = transactionBuilder.buildAssetParticipate(this.getAddress(), issuer, token, amount);
    return this.updateTransaction(transaction, latestBlock)
  }
}
